...
while True:
  for tieto in tiedot:
    try:
      ...
      if case in [1,2,6]:
        suunta = 'BUY'
        logging(3, tieto['symbol'],  'CASE: ' +  str(case), 'SMA1: ' + str(sma1_value) + '; SMA2: ' + str(sma2_value) , 1)
        logging(3, tieto['symbol'],  'TIETO', str(tieto), 1)
      elif case in [3,4,5]:
        suunta = 'SELL'
        logging(3, tieto['symbol'],  'CASE: ' +  str(case), 'SMA1: ' + str(sma1_value) + '; SMA2: ' + str(sma2_value) , 1)
        logging(3, tieto['symbol'],  'TIETO', str(tieto), 1)
      ...
      if case > 0 and ((case in [1,2,4,5] and suunta != tieto['last_action']) or (case in [3,6] and suunta == tieto['last_action'])):
        kauppalause = "client.create_order(symbol='" + str(tieto['symbol']) + "',side='" + str(suunta) + "',type='" + str(tyyppi)
        if suunta == 'BUY':
